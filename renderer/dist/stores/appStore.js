...
ipcRenderer.on(MessageTypes.UpdateCalendars, (event, arg) => {
    const parsedArgs = JSON.parse(arg);
    appStore.dispatch(updateCalendars(parsedArgs.calendars));
});
ipcRenderer.on(MessageTypes.UpdateContacts, (event, arg) => {
    const parsedArgs = JSON.parse(arg);
    appStore.dispatch(updateContacts(parsedArgs.contacts));
});
ipcRenderer.on(MessageTypes.UpdateQueue, (event, arg) => {
    const parsedArgs = JSON.parse(arg);
