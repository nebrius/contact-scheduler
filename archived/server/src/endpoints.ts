...
import { isUserRegistered, getUser, setPushSubscription, getContacts, setContacts } from './db';
import { processNotifications, rescheduleCurrentNotification, respondToCurrentNotification } from './notifications';
...
export function init(cb: CB): void {
  ...
  app.get('/', auth.createMiddleware(true), (req, res) => {
    ...
    res.render('index', {
      ...
      user: JSON.stringify(getUser((req as IRequest).userId))
    });
  });
  ...
  app.post('/api/rescheduleNotification', auth.createMiddleware(false), (req, res) => {
    rescheduleCurrentNotification((req as IRequest).userId, (err) => {
      if (err) {
        console.error(err);
        res.sendStatus(500);
      } else {
  ...
  app.post('/api/respondToNotification', auth.createMiddleware(false), (req, res) => {
    respondToCurrentNotification((req as IRequest).userId, (err) => {
      if (err) {
        console.error(err);
        res.sendStatus(500);
      } else {
